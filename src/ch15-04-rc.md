...
1. there are cases when a single value might have multiple owners.
2. You have to enable multiple ownership explicitly by using the Rust type Rc<T>, which is an abbreviation for reference counting.
   > 使用起来就是 python 的内存管理机制
...
4. Using Rc<T> to share data

![](https://kuanhsiaokuo.github.io/the-rust-programming-book-khk/img/trpl15-03.svg)
...
- we’ll change `main` so it has an inner scope around list `c`;
...
- we’ll see what `weak_count` is used for in the
